...
Outputs land at `status: draft`. Promotion is human-driven via `wb.publish` / `wb.approve`. Lost in the pipeline? Run `wb.wtd` or read [Workflows](https://amit-t.github.io/ai-workbench/workflows.html).
...
| `/wtd` | Cross | One next-action command per epic. Trimmer cousin of pmo-status. |
