...
### Ralph adapter V2 — E1 skill rewiring (2026-04-27)
- `skills/ralph-workspace-plan/SKILL.md`: rewrote to invoke `wb.ralph-plan` instead of `./scripts/ralph-plan.sh` directly. Added the mode resolver (`--mode workspace | per-repo | auto`), the `--engine` / `--thinking` / `--dry-run` flags, the `wb.ralph-enable-check` preflight callout, and the workspace rollup pointing at `repos/.ralph/fix_plan.md` sections in workspace mode.
- `skills/ralph-dispatch/SKILL.md`: rewrote to invoke `wb.ralph-dispatch` (with `--parallel`, `--engine`, `--status`, `--dry-run`). Removed the obsolete `--repos` and `--agent` flags from the V1 script-path call. Replaced the per-repo PID/log narrative with the ralph-owned `repos/.ralph/logs/parallel/` reality and the `wb.ralph-dispatch --status` output contract. Added an explicit "single-repo debug is a one-liner: `(cd $WB_ROOT/repos/<name> && ralph --live --monitor)`" section. Surfaced E2 (`--repos` filter) as a parked follow-up so users do not assume the flag is wired.
- `AGENTS.md`: dropped the stale `wb.ralph-loop` reference; replaced with `wb.ralph-dispatch` + the single-repo debug one-liner.
- `.ralph/fix_plan.md`: E1 marked done with learning notes; E1's last-line PR URL slot reserved.
- Smoke 22/22 + steering-lint clean (no script behavior changed; pure docs ship).
